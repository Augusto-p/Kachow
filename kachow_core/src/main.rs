...
use crate::{
    database::{Database, identity::LocalIdentity}, ipc::handle::handle_ipc_request, mdns::MdnsManager, state::KachowState, web::WEB,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let storage = Arc::new(
        Database::open("kachow.db")?
    );

    load_identity_or_default(storage.clone()).await;

    let state = Arc::new(
        KachowState::new(storage.clone())
    );

    let tcp_port = storage
        .get_identity_tcp_port()
        .await
        .unwrap_or(9533);
    ...
    println!(
        "Kachow Web escuchando en 0.0.0.0:{}",
        tcp_port
    );
    ...
    let ipc_task = tokio::spawn(async move {
        if let Err(err) = IpcServer::listen(
            &ipc_socket_path,
            move |req| {
                let state = ipc_state.clone();

                async move {
                    handle_ipc_request(req, state).await
                }
            },
        )
        .await
    ...
    let mdns_task = tokio::spawn(async move {
        loop {
            ...
            let instance_name = if is_mode_active {
                &("kachow-".to_string() + &mdns_state.storage.get_identity_device_id().await.unwrap_or_else(|| "".into()))
            } else {
                &mdns_state.storage.get_identity_secret_service_name().await.unwrap_or_else(|| "".into())
            };
